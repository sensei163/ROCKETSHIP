...
cfit_fit       = plot_data.cfit_fit;
cfit_gof       = plot_data.cfit_gof;
cfit_output    = plot_data.cfit_output;
...
if strcmp(plot_data.model_name,'aif') || strcmp(plot_data.model_name,'tofts')
    ...
    [ci, y] = predint(cfit_fit, [plot_data.timer plot_data.Cp'], 0.95, 'functional', 'on');
...
%     set(p_ci(2),'Color','k');
     ...
     hold on
    p_ci = plot(plot_data.timer,ci(:,1),...
        plot_data.timer,ci(:,2));
    set(p_ci(1),'Color','r');
    set(p_ci(2),'Color','r');
end
hold off
