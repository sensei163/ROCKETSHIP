...
if(neuroecon)
   ...
   % for k = 1:totale
        ...
        set(jj, 'MaximumNumberOfWorkers', CPU)
        set(jj, 'MinimumNumberOfWorkers', CPU)
...
save(fullfile(fileparts(directory1), [rootname 'AIF_with_vpFIT_voxels.mat']), 'x', 'tumind','dynamname', 'directory1' , 'xdata')
...
save_nii(make_nii(KtransROI, res(2:4), [1 1 1]), fullfile(fileparts(directory1), [actual '_Ktrans_withvp.nii']));
save_nii(make_nii(veROI, res(2:4), [1 1 1]), fullfile(fileparts(directory1),[actual '_ve_withvp.nii']));
save_nii(make_nii(vpROI, res(2:4), [1 1 1]), fullfile(fileparts(directory1),[actual '_vp_withvp.nii']));
