%% C_fit_AIFwithvp_COH:
%{
Fit to Standard Model

Required:

smoothcurve.m % Smooth out the tumor ROI curve
FXLStep1AIFhelper.m % Wrapper function for SM1 fitting
FXLStep1AIFcfit
...
tic
ROIX = FXLStep1AIFhelper(xdata,1)
toc
...
% problem = createOptimProblem('lsqcurvefit', 'x0', ROIX(1:2), 'objective',@FXLStep1AIF, 'lb', [ 0 0], 'ub', [1 1 ], 'xdata', xdata, 'options', options, 'ydata', CtROI' );
...
title(['2 compartment model vp: Ktrans: ' num2str(ROIX(1)) ' ve: ' num2str(ROIX(2))] ), ylabel('Ct (mM)'), xlabel('time (min)')
...
numvoxels      = size(Ct,2);
%res = [0 0.25 0.25 2];
save(fullfile(PathName1, [rootname 'AIF_no_vpFIT_ROI.mat']), 'res', 'currentimg', 'dynamname', 'lvind', 'tumind', 'ROIX', 'xdata', 'numvoxels');
