...
TR = str2num(answer{1});
FA = str2num(answer{2});
inj= str2num(answer{3});
hct= str2num(answer{4});
r1 = str2num(answer{5});
SNR= str2num(answer{6});
timeres = str2num(answer{7});
fw = str2num(answer{8});
...
disp(['SNR filter using SNR = ' num2str(SNR) ' had filtered out: ' num2str(snrfilter) ' voxels.']);
...
Sss      = mean(Stotallv(round(inj(1)):round(inj(2)),:));
...
for j = 1:numel(T1LV)
    ...
    ScaleFactorlv = (1/T1LV(j)) - mean(R1tLV(round(inj(1)):round(inj(2)), j));
...
Ssstum = mean(Stotaltum(round(inj(1)):round(inj(2)),:));
Sstar    = ((1-exp(-TR./T1))./(1-cosd(FA).*exp(-TR./T1)));
Sttum = Stotaltum;
...
for j = 1:numel(T1TUM)
    % Scale Sss values to T1 values
    ScaleFactortum = (1/T1TUM(j)) - mean(R1tTOI(round(inj(1)):round(inj(2)), j));
...
% f.1 AIF
%if(~iterremove)
Cp = (R1tLV-repmat((1./T1LV)', [size(R1tLV, 1) 1]))./(r1*(1-hct));
%end
% f.2 Tumor
Ct = (R1tTOI-repmat((1./T1TUM)', [size(R1tTOI, 1) 1]))./r1;
...
deltaR1LV = R1tLV-repmat(mean(R1tLV(round(inj(1)):round(inj(2)), :), 1), [size(R1tLV,1) 1]);

deltaR1TOI= R1tTOI-repmat(mean(R1tTOI(round(inj(1)):round(inj(2)), :), 1), [size(R1tTOI,1) 1]);
...
filename = fullfile(PathName1, [rootname 'R1info.mat'])
